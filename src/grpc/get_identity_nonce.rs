...
use dapi_grpc::platform::v0::get_identity_nonce_response::{get_identity_nonce_response_v0, Version};
...
impl PlatformGRPCClient {
    pub async fn get_identity_nonce(&self, identifier: Identifier) -> Result<IdentityNonce, Error> {
        ...
        let result = execution_result
            .map(|execution_response| {
                let get_identity_nonce_response = execution_response.inner;

                match get_identity_nonce_response.version.unwrap() {
                    Version::V0(get_identity_nonce_response_v0) => {
                        let result = get_identity_nonce_response_v0.result.unwrap();
