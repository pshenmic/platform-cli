...
use dapi_grpc::platform::v0::get_identity_by_public_key_hash_response::{get_identity_by_public_key_hash_response_v0, Version};
    ...
    pub async fn get_identity_by_public_key_hash(&self, public_key_hash: PubkeyHash) -> Result<Identity, Error> {
        ...
        let result = execution_result
            .map(|execution_response| {
                let get_identity_by_public_key_hash_response = execution_response.inner;

                match get_identity_by_public_key_hash_response.version.unwrap() {
                    Version::V0(get_identity_by_public_key_hash_response_v0) => {
                        let result = get_identity_by_public_key_hash_response_v0.result.unwrap();
